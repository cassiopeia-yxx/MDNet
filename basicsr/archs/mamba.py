...
class SS2D(nn.Module, SS2Dv0, SS2Dv2, SS2Dv3):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
...
class DualSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ffn = FeedForward(dim = dim, ffn_expansion_factor=2.66, bias=False)
        self.fusion = nn.Conv2d(2*dim, dim, 1, 1, 0)
        # self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=mlp_act_layer, drop=mlp_drop_rate)
        
    def forward(self, input: torch.Tensor):
        x_norm =  self.norm1(input)
        x_global = self.long(x_norm)
        x_local = self.local(x_norm)
        x = self.drop_path(self.fusion(torch.cat((x_global, x_local), dim=1))) + input
